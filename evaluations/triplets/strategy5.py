...
def _normalize_entities(records: Sequence[EntityRecord]) -> dict[str, dict[str, object]]:
    ...
    for record in records:
        ...
        entry: dict[str, object] = {"type": _normalize_relation(record.type) or "OTHER"}
        if aliases:
...
def _build_alias_to_canonical(
    ...
    for canonical, entry in entities.items():
        for variant in {canonical, canonical.upper(), canonical.lower()}:
...
def _resolve_canonical_entity(
    ...
    *,
    ...
    if not normalized:
        return ""
    key = _entity_alias_key(normalized)
...
def _assemble_payload(*, article_name: str, extracted: ExtractionResult) -> dict[str, object]:
...
def _build_prompt(article_text: str) -> str:
    return (
        ...
        "1) Do NOT use any predefined relation ontology.\n"
        "2) Choose relation labels freely based on article semantics.\n"
        "3) fact_edges: explicit supported statements only.\n"
        "4) derived_edges: inferred or multi-hop implications from the text/model.\n"
        "5) contradicted_edges: explicit refuted/retracted/disproven claims.\n"
        "6) hypothesis_only: speculative/unproven claims only.\n"
        "7) Keep entities canonical and include aliases when useful.\n"
...
async def _predict_article_with_agent(
    *,
    article_path: Path,
    model: str,
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="artana_triplets_strategy5_"))
    ...
    context_builder = ContextBuilder(
        identity=(
            "You are an autonomous scientific IE agent. "
            "Extract factual, derived, contradicted, and hypothetical graph edges."
        ),
    ...
    try:
        extracted = await agent.run(
            run_id=f"triplets_strategy5::{article_path.stem}",
            tenant=tenant,
            model=model,
            system_prompt=(
        ...
        return _assemble_payload(article_name=article_path.stem, extracted=extracted)
    finally:
        await kernel.close()
...
async def _run(
    *,
    ...
    for article_path in article_paths:
        predicted = await _predict_article_with_agent(
            article_path=article_path,
            model=model,
            tenant=tenant,
        ...
        if output_dir is not None:
            output_path = output_dir / f"{article_path.stem}.strategy5.json"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Strategy5: unconstrained AutonomousAgent relation extraction."
    )
    parser.add_argument(
        "--article",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Max AutonomousAgent iterations for extraction.",
    )
    parser.add_argument(
        "--reasoning-effort",
...
async def _main_async() -> int:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise RuntimeError(
            "OPENAI_API_KEY is required for strategy5 execution. "
