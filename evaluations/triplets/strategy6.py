...
async def _predict_article_with_agent(
    *,
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="artana_triplets_strategy6_"))
    ...
    context_builder = ContextBuilder(
        identity=(
            ...
            "Extract then verify structured biomedical graph edges."
        ),
        progressive_skills=False,
    )
    agent = AutonomousAgent(kernel=kernel, context_builder=context_builder)

    try:
        free_result = await agent.run(
            run_id=f"triplets_strategy6::{article_path.stem}::extract",
            tenant=tenant,
            model=extract_model,
            system_prompt=(
            ...
            max_iterations=extract_iterations,
        )

        reviewed_result = await agent.run(
            run_id=f"triplets_strategy6::{article_path.stem}::review",
            tenant=tenant,
            model=review_model,
            system_prompt=(
                "Return only structured JSON matching the schema. "
    ...
    *,
    ...
    for article_path in article_paths:
        predicted = await _predict_article_with_agent(
            ...
            tenant=tenant,
            extract_iterations=extract_iterations,
        ...
        if output_dir is not None:
            output_path = output_dir / f"{article_path.stem}.strategy6.json"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Strategy6: free extraction + review/normalize + evidence gating."
    )
    parser.add_argument(
        "--article",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "else ARTANA_MODEL, else gpt-5-mini)."
        ),
    )
    parser.add_argument(
        "--review-model",
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Max AutonomousAgent iterations for free extraction pass.",
    )
    parser.add_argument(
        "--review-iterations",
        type=int,
        default=8,
        help="Max AutonomousAgent iterations for review/normalization pass.",
    )
    parser.add_argument(
        "--reasoning-effort",
...
async def _main_async() -> int:
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise RuntimeError(
            "OPENAI_API_KEY is required for strategy6 execution. "
    ...
    if args.extract_iterations <= 0:
        raise ValueError("--extract-iterations must be >= 1.")
    if args.review_iterations <= 0:
        raise ValueError("--review-iterations must be >= 1.")
    ...
    extract_model = args.extract_model or fallback_model or "gpt-5-mini"
    ...
    results = await _run(
        ...
        extract_iterations=args.extract_iterations,
