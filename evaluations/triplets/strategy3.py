...
class CanonicalGraphResult(BaseModel):
    benchmark_name: str = ""
    entities: list[EntityRecord] = Field(default_factory=list)
    fact_edges: list[Triplet] = Field(default_factory=list)
    contradicted_edges: list[Triplet] = Field(default_factory=list)
...
def _build_canonicalization_prompt(
    *,
    ...
    raw_payload = {
        "benchmark_name": raw_extraction.benchmark_name,
        "entities": [item.model_dump() for item in raw_extraction.entities],
        "fact_edges": [edge.model_dump() for edge in raw_extraction.fact_edges],
        "contradicted_edges": [
            edge.model_dump() for edge in raw_extraction.contradicted_edges
        ],
        "hypothesis_only": [edge.model_dump() for edge in raw_extraction.hypothesis_only],
    }
    return (
        "Normalize the extracted graph into canonical entities and canonical edges.\n\n"
        "Requirements:\n"
        "1) Keep only claims supported by the raw extraction and article text.\n"
        "2) Canonicalize aliases to one canonical_name per concept.\n"
        "3) Canonical names should not include parenthetical alias text.\n"
        "4) For symbolic/code-like entities, prefer concise uppercase symbolic forms.\n"
        "5) Keep descriptive multi-word entity names as stable phrases.\n"
        "6) Preserve epistemic partitions: fact_edges, contradicted_edges, hypothesis_only.\n"
        "7) Use only allowed relation labels from the schema.\n"
        "8) Do not derive multi-hop edges in this step.\n"
        "9) Prefer precision over recall when ambiguous.\n\n"
        f"ARTICLE:\n{article_text.strip()}\n\n"
        f"RAW_EXTRACTION_JSON:\n{json.dumps(raw_payload, ensure_ascii=False)}"
    )
...
def _build_derived_adjudication_prompt(
    *,
    ...
    return (
        ...
        "4) Prefer endpoint-oriented derived edges (process-level or disease-level),\n"
        "   and avoid trivial restatements on intermediate nodes.\n"
        "5) Use canonical entity names exactly as defined in EXPLICIT_GRAPH_JSON entities.\n"
        "6) Use only relation labels:\n"
        "   DERIVED_REGULATES, DERIVED_ASSOCIATED_WITH, DERIVED_DOWNREGULATES,\n"
        "   DERIVED_REDUCES, DERIVED_INHIBITS.\n"
        "7) Do not output duplicates.\n\n"
        f"ARTICLE:\n{article_text.strip()}\n\n"
...
def _is_symbolic_entity_name(value: str) -> bool:
    token = value.strip()
    if " " in token:
        return False
    if not token:
    ...
    has_code = any(char.isdigit() for char in token) or "-" in token or "_" in token
    return has_alpha and (has_upper or has_code)
...
def _generate_deterministic_derived_candidates(
    *,
    ...
    for source, rels in outgoing_by_source.items():
        ...
        if source_type in {"GENE", "DRUG", "PROCESS"}:
...
def _assemble_payload(
    *,
    ...
    allowed_derived_candidates: list[list[str]] | None = None,
) -> dict[str, object]:
    ...
    if allowed_derived_candidates is not None:
        candidate_set: set[tuple[str, str, str]] = set()
        for item in allowed_derived_candidates:
            if len(item) != 3:
                continue
            src, relation, dst = item
            if not all(isinstance(part, str) for part in (src, relation, dst)):
                continue
            candidate_set.add((src, relation, dst))
        derived_edges = [
            edge
            for edge in derived_edges
            if (edge[0], edge[1], edge[2]) in candidate_set
        ]
...
def _compare_with_gold(
    *,
    ...
    keys_to_score: tuple[str, ...]
    if score_mode == "all":
        keys_to_score = EDGE_KEYS
    else:
        keys_to_score = tuple(key for key in EDGE_KEYS if key in gold)
        if not keys_to_score:
            keys_to_score = ("fact_edges", "derived_edges")
...
async def _predict_article_with_autonomous_agent(
    *,
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="artana_triplets_strategy3_"))
    ...
    try:
        raw_extraction = await agent.run(
            run_id=f"triplets_strategy3::{article_path.stem}::extract_raw",
            tenant=tenant,
            model=model,
            system_prompt=(
        ...
        canonical_graph = await agent.run(
            run_id=f"triplets_strategy3::{article_path.stem}::canonicalize",
            tenant=tenant,
            model=model,
            system_prompt=(
                "Return only structured JSON matching the output schema. "
            ...
            prompt=_build_canonicalization_prompt(
                article_text=article_text,
                raw_extraction=raw_extraction,
            ),
            output_schema=CanonicalGraphResult,
            max_iterations=agent_iterations,
        )

        entities = _normalize_entities(canonical_graph.entities)
        alias_to_canonical, ranked_alias_keys = _build_alias_to_canonical(entities)
        normalized_facts = _normalize_triplets(
            canonical_graph.fact_edges,
            alias_to_canonical=alias_to_canonical,
            ranked_alias_keys=ranked_alias_keys,
            allowed_relations=FACT_RELATIONS,
        )
        normalized_contradictions = _normalize_triplets(
            canonical_graph.contradicted_edges,
            alias_to_canonical=alias_to_canonical,
            ranked_alias_keys=ranked_alias_keys,
            allowed_relations=FACT_RELATIONS,
        )
        normalized_hypotheses = _normalize_triplets(
            canonical_graph.hypothesis_only,
            alias_to_canonical=alias_to_canonical,
            ranked_alias_keys=ranked_alias_keys,
            allowed_relations=frozenset({HYPOTHESIS_RELATION}),
            default_relation=HYPOTHESIS_RELATION,
        ...
        derived = await agent.run(
            run_id=f"triplets_strategy3::{article_path.stem}::derive",
            tenant=tenant,
            model=model,
            system_prompt=(
                "Return only structured JSON matching the output schema. "
        ...
        return _assemble_payload(
            ...
            extracted=ExtractionResult.model_validate(canonical_graph.model_dump()),
            derived=derived,
            allowed_derived_candidates=deterministic_candidates,
        )
    finally:
        await kernel.close()
...
async def _run(
    *,
    ...
    for article_path in article_paths:
        ...
        if output_dir is not None:
            output_path = output_dir / f"{article_path.stem}.strategy3.json"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Strategy3: AutonomousAgent + canonicalization + constrained derivation."
    )
    parser.add_argument(
        "--article",
        type=Path,
    ...
    parser.add_argument(
        "--model",
        type=str,
        default=os.getenv("ARTANA_MODEL", "gpt-5.2"),
        help="Model id for LiteLLMAdapter (default: ARTANA_MODEL or gpt-5.2).",
    )
    parser.add_argument(
        "--tenant-id",
        type=str,
